using Dates
using ModelSelection: Preprocessing, AllSubsetRegression, CrossValidation, ModelSelectionData
...
mutable struct ModelSelectionJob
    ...
    function ModelSelectionJob(filename::String, tempfile::String, hash::String, payload::Dict{Symbol ,Any})
        ...
        id = string(uuid4())
        time_enqueued = now()
        new(id, filename, tempfile, hash, payload, PENDING, time_enqueued, nothing, nothing, nothing, estimator, equation, nothing)
    end
end
